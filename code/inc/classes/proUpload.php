 ...
 * Please help out by contacting out some bugs/flaws if you encounter any. Thanks!
 *
 * @category    Image uploader
 * @package     ProUpload
 * @version     1.0.0
 * @author      Kerul Patel 
 * @link        http://www.prodesignz.net
 */
 
/**
	Usage:
	require_once  "src\proUpload.php";
	...
	Custom Upload With Size and Dimension Restriction
	=================================================
    ...
    ->shrink(["height"=>100, "width"=>200],true) // shrink to 100*200 pixels and keep aspect ratio true
...
class ProUpload
{
     ...
     * @var array
     */
    protected $imageSize = array("min" => 1, "max" =>18000000);
 ...
 /**
     ...
     * @throws ImageUploaderException
     */
    protected function moveApplyShrink($fileName, $imageName)
    {
        ...
        $oldImage = $this->getPixels($fileName);
        $newImage = $this->getNewImageSize($oldImage, false);
    ...
    /**
     * Without uploading, just crop/watermark/shrink all images in to other folders
     *
     * @param $action - the task.. ex: 'crop', 'watermark', 'shrink'...
     * @param $source - Source Image.  Provide full path pls.
     * @param $imageName - the image you want to copy. Provide full path pls.
     * @throws ImageUploaderException
     */
    public function makecopy($action, $sourceImage, $imageName){

        if(empty($action) || !file_exists($sourceImage)){
            throw new ImageUploaderException(__FUNCTION__." needs two arguments. the Task and Image name");
        }
        ...
        if($action == "shrink" &&
            $this->shrinkImageTo)
        {
            $this->moveApplyShrink($sourceImage, $imageName);
            return true;
        }
        ...
        if($action == "crop" && 
            $this->cropImageTo)
        {
            $this->applyCrop($sourceImage, $imageName);
            return true;
        }
