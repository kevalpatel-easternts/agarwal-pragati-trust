...
class SimpleImage
{
    ...
    public function __destruct()
    {
        ...
        $type_check =
            gettype($this->image) == "object" &&
            get_class($this->image) == "GdImage";
    ...
    public function textBox($text, $options)
    {
        ...
        list($lines, $isLastLine, $lastLineHeight) = self::textSeparateLines(
            $text,
            $fontFile,
            $fontSize,
            $maxWidth
        ...
        $maxHeight =
            (count($lines) - 1) * ($fontSizePx * 1.2 + $leading) +
            $lastLineHeight;
    ...
    public function extractColors($count = 5, $backgroundColor = null)
    {
        ...
        $palette = \League\ColorExtractor\Palette::fromGD(
            $this->image,
            $backgroundColor
        );
        ...
        foreach ($colors as $key => $value) {
            $colors[$key] = self::normalizeColor(
                \League\ColorExtractor\Color::fromIntToHex($value)
            );
        }
...
try {
    ...
    $mime_content_type = mime_content_type($image1); // get path from url
    $path = parse_url($image1, PHP_URL_PATH); // get path from url
    ...
    // Manipulate it
    $image
        ->fromDataUri($dataUri) // load parrot.jpg
        ->autoOrient() // adjust orientation based on exif data
        // ->fitToWidth($_REQUEST["w"])
        // ->fitToHeight($_REQUEST["h"])
        // ->bestFit($_REQUEST["w"], $_REQUEST["h"]) // proportinoally resize to fit inside a 250x400 box
        ->resize($_REQUEST["w"], $_REQUEST["h"]) // resize to fit inside a your given height and width
        // ->crop(0,0,$_REQUEST["w"], $_REQUEST["h"]) // resize to fit inside a 250x400 box
