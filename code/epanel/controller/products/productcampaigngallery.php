...
class productcampaigngallery
{
	function addForm()
	{
		//echo "SELECT * from gallery where productID = '".(int)$_REQUEST['productID']."' and isFront = 'S'" ;
		$imgSql = ets_db_query("SELECT * from gallery where productID = '".(int)$_REQUEST['productID']."' and isFront = 'C'");
		$imgfiles = "";
		...
		var files = [
		   <?php echo $imgfiles; ?>
			...
			$('#fileupload').fileupload({
    				url: '<?php echo HTTP_SERVER.WS_ADMIN_ROOT; ?>uploadcampaigngallery.php?productID=<?php echo (int)$_REQUEST['productID']; ?>'
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-upload">
        ...
        <td>
			    ...
			    <input name="isFront[]" type="hidden" value="C" class="form-control">
			</label>
	   </td>
    ...
    <tr class="template-download">
        ...
        <td>
         ...
         <td>
			    ...
			    <input name="isFront[]" type="hidden" value="C">
			</label>
	   </td>
...
<?php
	...
	{		
		...
		if(ets_db_num_rows($qry) > 0){
			...
			$form->addElement(new Element_Hidden("action", "productcampaigngallery"));
			...
			$form->addElement(new Element_Hidden("isFront", 'C'));
			...
			$form->addElement(new Element_HTML('<img src="'.DIR_WS_PROJECT_PATH.$rs['productID']."/campaign/thumbnail/".$rs['galleryImage'].'" style="margin-left:200px;">'));
	...
	function edit()
	{
		...
		$path = DIR_FS_PROJECT_PATH.$_POST['productID'].'/campaign/';
		if(!file_exists($path))
			{
				mkdir($path);
		...
		if($updimg){
			@unlink(DIR_FS_PROJECT_PATH.$_POST['productID']."/campaign/".$_POST['prevImage']);
			...
			else 
			{
						...
						$mresult = $image->shrink(array("height"=>480, "width"=>480),true)
							->makecopy('shrink',$target_path,DIR_FS_PROJECT_PATH.$_POST['productID']."/campaign/".$_FILES["galleryImage"]["name"].'_mobile.jpg');
						}
...
<script>
$(document).ready(function() {
	var listURL = "helperfunc/projectcampaigngalleryList.php?productID=<?php echo (int)$_REQUEST['productID'];?>";
