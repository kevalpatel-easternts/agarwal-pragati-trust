...
class productgallery
{
	function addForm()
	{
		...
		$imgSql = ets_db_query("Select * from productImages where productID = '".(int)$_REQUEST['productID']."' and isFront = 'G' ");
		$imgfiles = "";
		...
		var files = [
		   <?php echo $imgfiles; ?>
			...
			$('#fileupload').fileupload({
    				url: '<?php echo HTTP_SERVER.WS_ADMIN_ROOT; ?>uploadgallery.php?productID=<?php echo (int)$_REQUEST['productID']; ?>'
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-upload">
        ...
        <td>
			    ...
			    <input name="isFront[]" type="hidden" value="G" class="form-control">
			</label>
	   </td>
    ...
    <tr class="template-download">
        ...
        <td>
         ...
         <td>
			    ...
			    <input name="isFront[]" type="hidden" value="G">
			</label>
	   </td>
...
<?php
	...
	{		
		...
		if(ets_db_num_rows($qry) > 0){
			...
			$form->addElement(new Element_Hidden("action", "productgallery"));
			...
			$form->addElement(new Element_Hidden("isFront", 'G'));
