...
class album
{
	...
	{   
		...
		$path = DIR_FS_application_PATH.$album_id.'/';
		if(!file_exists($path))	{
			mkdir($path);
			//exec("chown ".FILEUSER.FILEUSER." ".$path);
			//exec("chmod 0777 ".$path);
		}
		insSeoLnk($album_id,"application",$album_slug);
		return true;
	}
	function edit()
	{  
		...
		if(ets_db_query($updqry)){
			updSeoLnk($_POST['a_id'],"application",$album_slug);
			$album_id = $_POST['a_id'];
			$path = DIR_FS_application_PATH.$album_id.'/';
			if(!file_exists($path))	{
				mkdir($path);
	...
	{	
		...
		delSeoLnk($_GET['a_id'],'application');
		$delsql = "Delete from album where a_id= '".$_GET['a_id']."'";
		$delqry = ets_db_query($delsql) or die(ets_db_error().$delsql);
		
		delete_folder(DIR_FS_application_PATH.$_GET['a_id']);
		
		$qry = "Delete from application_master where a_id = '".$_GET['a_id']."'";
