...
class albumtype
{
	...
	{   
		...
		insSeoLnk($albumtype_id,"application",$albumtype_slug);
		return true;
	}
	function editForm()
	{		
	...
	$qry = "Select * from application_master where a_id in (Select a_id from album where album_type_id = '".$_GET['type_id']."')";
	$res = ets_db_query($qry);
	echo $qry;
	
	while($arr = ets_db_fetch_array($res))
	{
		delete_folder(DIR_FS_application_PATH.$arr['a_id']);
		delSeoLnk($arr['a_id'],'application');
	}
	
	$qry = "Delete from application_master where a_id in (Select a_id from album where album_type_id = '".$_GET['type_id']."')";
	$res = ets_db_query($qry);
	...
	delSeoLnk($_GET['type_id'],"application");
