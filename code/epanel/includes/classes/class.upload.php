            ...
            if ($image_manipulation) {
                ...
                if ($this->processed && $image_src) {
                   ...
                   if ($gd_version >= 2 && (is_numeric($this->image_overlay_percent) && !empty($this->image_overlay_color))) {
					...
					// add colorise
                   if ($gd_version >= 2 && !empty($this->image_colorise)) {
                        $this->log .= '- ' . _("apply colorise") . '<br />';
						
                        sscanf($this->image_colorise, "#%2x%2x%2x", $red, $green, $blue);
						$tmp = $this->create_image($this->image_dst_y, $this->image_dst_x, $image_dst);
                        //$filter=imagecreatetruecolor($this->image_dst_x, $this->image_dst_y);
                        $tmp=imagefilter($tmp, IMG_FILTER_COLORIZE , $red, $green, $blue);
                        //imagefilledrectangle($filter, 0, 0, $this->image_dst_x, $this->image_dst_y, $color);
                        $image_dst = $this->transfer_image($tmp, $image_dst);
                        imagedestroy($tmp);
                    }
