<?php
	...
	$queryString =  ets_db_query("SELECT * from project_gallery where projectID = '".(int)$_REQUEST['projectID']."' and type = 'G' order by sortorder") or die(ets_db_error());	
    ...
    width: 100px;"></td>';
				...
				$Action='<td><a href="index.php?controller=projects&action=projectimages&subaction=editForm&galleryID='.$res['galleryID'].'&projectID='.$res['projectID'].'" title="Edit" class="btn btn-success"><i class="fa fa-edit"></i></a> | 
				<a href="index.php?controller=projects&action=projectimages&subaction=delete&galleryID='.$res['galleryID'].'&projectID='.$res['projectID'].'" title="Delete" class="btn btn-danger" onClick="return confirmSubmit();" ><i class="fa fa-times"></i></a>
				</td>';
